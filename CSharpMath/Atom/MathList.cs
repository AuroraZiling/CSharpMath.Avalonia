...
internal class DisabledMathList : MathList {
...
#pragma warning restore CA1710 // Identifiers should have correct suffix
    public List<MathAtom> Atoms { get; private init; }
    public MathList() => Atoms = [];
    public MathList(IEnumerable<MathAtom> atoms) => Atoms = [..atoms];
    public MathList(params MathAtom[] atoms) => Atoms = [..atoms];
    ...
    public bool EqualsList(MathList? otherList) {
        if (otherList == null || otherList.Count != Count) {
            return false;
        }
    ...
    public MathList Slice(int index, int count) => new() { Atoms = Atoms.GetRange(index, count) };
}